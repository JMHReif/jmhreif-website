...
Service1 will use port `8080`, then we can set Service2 to use a different one when we get there.
...
So service2 will use port `8081` and recall that service1 is using port `8080`. This means traffic for each application can communicate with one another without conflicting or getting sent to the wrong place.
