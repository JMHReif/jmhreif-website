...
At this step, it has decent reasoning, but no access to personal, private, or curated data. Everything beyond this point is about closing the gap between what the model can reason about and what it actually needs to know.

== Act 2: Agents + Tools

The natural next step is to give the model a way to *get* real data rather than invent it. The goal here is to direct some determinism into a non-deterministic system. This is where agents start to become concrete.
...
Early on, there was a common shortcut we used to use: describe what you want the model to do in the prompt. As an example, `if the user asks for recommendations, call this API by returning JSON in the following format: {...}`.

I built some of my earliest apps this way, and it worked well...until it didn't. In practice, this approach falls apart with hallucinated parameters, no schema enforcement, and inconsistent output format. This method tries to manipulate our way into the LLM, but it is extremely fragile. I think of the mantra "hope is not a strategy," and that's exactly what it feels like once you're debugging it in a real app.

The better approach is structured tool calling where you define tools explicitly with typed parameters and schemas. The model selects which tool to call based on the user's request, and the application executes it with consistent results. In Spring AI, this is done through `@Tool`-annotated methods.

The key shift is conceptual, not just technical. What I love about this approach is that the separation puts real control back in the hands of developers because we can design the processes, pipelines, and rules in the application. We can validate inputs, test tools independently, and trust that execution is deterministic (because it uses traditional functions/methods). The model brings output, and the application brings reliability.

The model didn't get smarter. Instead, we gave it a way to ask for help. Act 2 in the demo shows this directly. The same question that produced hallucinations in Act 1 now returns real, graph-traversed results from Neo4j. You get recommendations based on actual reading history, semantic review search, and graph-enriched queries.

== Act 3: Context is key

At this point, we have solved one important problem: the model can retrieve real information instead of inventing it. But it is not just context, but *smarter* context, that makes a difference. That's the real superpower, and it's the part that still needs work after Act 2. We have data access, but no memory over time.
...
Early on, we tried to solve this in the prompt by appending conversation history that passed it in with each request and let the model use it. That works until the context window fills up, and then things get expensive and slow. The issue is that even with tools, context still accumulates.

We don't need *more* context. We need the *right* information. More context doesn't always mean better answers, and sometimes it means worse ones because the model has to attend to everything even when most of it isn't relevant.

There is a more useful framing here: *memory is a system responsibility, not a model responsibility*.

Because LLMs are stateless, the application layer must decide what context to provide, when to retrieve it, and how to store it. Instead of asking "how do I make the model remember things", we can start asking "how do I build a retrieval layer that surfaces the right information at the right time?"
...
Instead of storing knowledge as disconnected chunks that the model is supposed to stitch together, we can give the system structure. There are several approaches to this problem, but graph databases fit well. Memory has two distinct forms:
...
Acts 2 and 3 produce a genuinely useful system. But as you add capability, you start digging some holes...the current application has great context, but it is tightly integrated. It can be hard to switch gears and make changes.
...
Tight coupling that feels fine for one app, but creates a maintenance headache as things grow.

=== What MCP changes

MCP (Model Context Protocol) is an open protocol that addresses this at the architecture level. Instead of hard-wiring tool definitions into each application, a server exposes capabilities. It defines what tools exist, what they do, and what parameters they take. Then clients (agents, applications, whatever speaks the protocol) discover and invoke those tools dynamically.
...
In Act 4, the underlying queries do not change at all compared to Acts 2 and 3, but the agent discovers the available tools at runtime. They are not hard-coded. The architecture changes, not the business logic. Tools are exposed through an MCP server instead of being wired directly into the application. Same logic, less coupling. I didn't rewrite the application code. I changed where the tools live.
